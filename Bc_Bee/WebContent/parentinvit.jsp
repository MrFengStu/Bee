...
<body>
	<div id="body">
		<div id="header">
			<div class="wrapper">
				<a class="logo" href="${ctx }/index.jsp"> <img width="229"
					height="43" alt="拉勾招聘-专注互联网招聘" src="${ctx }/style/images/logo.png">
				</a>
				...
				<dl class="collapsible_menu">
					...
					<dd style="display: none;">
						<a rel="nofollow" href="${ctx }/jiazhanginfo.jsp">我的信息</a>
					</dd>
					<dd style="display: none;">
						<a href="${ctx }/Delivery/jiazhangInit">我收到的简历 <span id="noticeNo" class="red dn">(0)</span></a>
					</dd>
							...
							<c:forEach items="${invitationmap }" var="ms">
								<li>
									<div class="d_item">
										...
										<div class="clear"></div>
										<div class="d_resume">
											结算方式： <span>${ms.value.settle }</span>
										</div>
										<a class="btn_showprogress"
											href="${ctx }/Invitation/resumedelete?InId=${ms.key.inId}">删除</a>
								</li>
							</c:forEach>
						</ul>
						<input type="hidden" value="-1" name="tag" /> <input type="hidden"
							value="" name="r" />
					</form>
					
					<div class="Pagination">
						<td colspan="6" align="center">共${invitationPage.totalCount }条记录
							共${invitationPage.totalPageNum }页	当前第${invitationPage.currentPageNum }页<br /> 
							<a	href="${ctx }/Invitation/send?pageNum=1" name="firstPage" value="首页">首页</a> 
							<c:if test="${invitationPage.currentPageNum!=1 }">
								<a	href="${ctx }/Invitation/send?pageNum=${invitationPage.prePageNum}"	name="firstPage" value="首页">上一页</a>
							</c:if> 
							<c:forEach begin="1" end="${invitationPage.totalPageNum }" var="pageNum">
								<a name="pagen"	href="${ctx }/Invitation/send?pageNum=${pageNum }">${pageNum }</a>
							</c:forEach> 
							<c:if test="${invitationPage.currentPageNum!=invitationPage.totalPageNum }">
								<a	href="${ctx }/Invitation/send?pageNum=${invitationPage.nextPageNum}" name="firstPage" value="首页">下一页</a>
							</c:if> 
							<a	href="${ctx }/Invitation/send?pageNum=${invitationPage.totalPageNum}" name="firstPage" value="首页">尾页</a>
						</td>
					</div>
				</div>
				<div class="content_r">
					...
					<div class="mycenterR" id="myInfo">
						...
						<table>
							<tr>
								...
								<td><a target="_blank" href="${ctx }/subscribe.html"> <img
					...
					<div class="greybg qrcode mt20">
						<img width="242" height="242" alt="拉勾微信公众号二维码"
							src="${ctx }/style/images/qrcode.jpg"/> <span class="c7">扫描Bee二维码，微信轻松搜工作</span>
					</div>
				</div>
			</div>
